...
const fileInput = "./file_inputs.txt";
...
const parking = (data) => {
  ...
  const carNumber = data.trim().replace(TYPE_CONSTANTS.PARK, "");
...
const parkingLotProjectOutput = () => {
  const convertFileIntoArray = fs
    .readFileSync(fileInput)
    .toString()
    .split("\n");
  ...
  convertFileIntoArray.map((data) => {
    ...
    if (
      conditionType(data, TYPE_CONSTANTS.PARK) &&
      !conditionType(data, TYPE_CONSTANTS.CREATE_LOT)
    ) {
      parking(data);
    }
